...
#ifndef lint
static  char Sccs_Id[] = "@(#)$Id: fm_cust_pol_prep_payinfo.c /cgbubrm_7.5.0.rwsmod/6 2014/06/20 12:59:18 praghura Exp $";
#endif
	...
	pin_errbuf_t		*ebufp)
{
	...
	pin_flist_t     *ro_i_flistp = NULL;
        pin_flist_t     *acc_flistp = NULL;	
        pin_flist_t     *ro_o_flistpp = NULL;	
	poid_t          *pdp = NULL;
	poid_t          *ipdp = NULL;
	poid_t          *new_pdp = NULL;
	...
	u_int64         database = 0;
	u_int           *pay_type = NULL;
	u_int           *input_inv_type = NULL;
	u_int           *acc_inv_type = NULL;
	u_int           *type = NULL;
	int32           option = 0;
	time_t		due_t = 0;
	u_int		due_dom = 0;
	...
	if (pay_pdp && (PIN_POID_IS_TYPE_ONLY(pay_pdp))) {
         ...
         * Identify the bill type and then fill in the missing fields
           ...
           steps
         ***********************************************************/
	// Summary Invoice Change Niladri


	ro_i_flistp = PIN_FLIST_CREATE(ebufp);

        acc_flistp = PIN_FLIST_FLD_GET(in_flistp, PIN_FLD_ACCOUNT_OBJ, 0, ebufp);
        
	input_inv_type = (u_int *) PIN_FLIST_FLD_GET(in_flistp, PIN_FLD_INV_TYPE, 1, ebufp);
	PIN_FLIST_FLD_PUT(flistp, PIN_FLD_POID, acc_flistp, ebufp);

	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,	"PCM_OP_READ_OBJ input flist", ro_i_flistp);

	PCM_OP(ctxp, PCM_OP_READ_OBJ, 0, ro_i_flistp, &ro_o_flistpp, ebufp);

	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,	"PCM_OP_READ_OBJ return flist", ro_o_flistpp);

	acc_inv_type = (u_int *) PIN_FLIST_FLD_GET(ro_o_flistpp, PIN_FLD_INV_TYPE, 1, ebufp);
	acc_cust_seg =  PIN_FLIST_FLD_GET(ro_o_flistpp, PIN_FLD_CUSTOMER_SEGMENT_LIST, 1, ebufp);

	if( strcmp(acc_inv_type, input_inv_type)!= 0)

	{
		PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_ERROR,"Invoice Type Changed in Update Custo0mer");
	}

	// Summary Invoice change end Niladri
	...
	pin_errbuf_t	*ebufp)
{
	...
	if ((vp == (void *)NULL) || (strlen(vp) == 0)) {
		...
		if (!vp && (PIN_POID_IS_TYPE_ONLY(pdp))) {
			...
			sprintf(item_no, "%s%s%04d", sequence_id, separator, item_seq);
