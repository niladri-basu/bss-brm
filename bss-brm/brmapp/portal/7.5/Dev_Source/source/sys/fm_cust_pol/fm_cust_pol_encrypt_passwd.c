...
#ifndef lint
static  char Sccs_Id[] = "@(#)$Id: fm_cust_pol_encrypt_passwd.c /cgbubrm_7.5.0.rwsmod/7 2014/10/21 01:57:50 sagaur Exp $";
#endif
	...
	if (pwd_clear) {
		if (fm_utils_is_subtype(o_pdp, "/service/ip")||
			...
			if( flags == 1 ) {
				...
				if (strncmp(pwd_in_db, "md5|", 4) == 0) {
					pwd_crypt = fm_cust_pol_encrypt_md5(pwd_clear, &(err));
				}
				else { // sha1 encoded format
					//salt = strtok(pwd_in_db, "|");
					//if(salt != NULL) {
						// get the salt which is after second token
						//salt = strtok(NULL, "|");
					//}
				...
				pwd_crypt = (char *)sha1_encode(pwd_clear,NULL);
