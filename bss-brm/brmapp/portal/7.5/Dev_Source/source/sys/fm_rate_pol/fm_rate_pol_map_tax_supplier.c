...
#ifndef lint
static  char    Sccs_id[] = "@(#)$Id: fm_rate_pol_map_tax_supplier.c /cgbubrm_7.5.0.idcmod/1 2014/01/07 14:37:07 amandavi Exp $";
#endif
	...
	pin_errbuf_t		*ebufp)
{
	...
	*o_flistpp = PIN_FLIST_CREATE(ebufp);
	...
	if (bparam_location_mode_for_taxation == PIN_RATE_LOC_GEOCODE 
		...
		buffer = pin_malloc(strlen(ship_from) + strlen(geocode) + 8);
		if (buffer == NULL) {
			pin_set_err(ebufp, PIN_ERRLOC_FM,
				PIN_ERRCLASS_SYSTEM_DETERMINATE,
		...
		sprintf(buffer, "%s;[%s,1,1]", ship_from, geocode);
	}
