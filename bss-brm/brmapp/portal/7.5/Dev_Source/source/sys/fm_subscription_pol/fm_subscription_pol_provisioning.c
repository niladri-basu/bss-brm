...
#ifndef lint
static	char	Sccs_id[] = "@(#)$Id: fm_subscription_pol_provisioning.c /cgbubrm_7.5.0.rwsmod/2 2014/07/23 16:39:46 vivilin Exp $";
#endif
	...
	char *tag, pin_errbuf_t *ebufp)
{
	...
	if (buy) {
		...
		if ((strlen(spool) + strlen(login)) >= (sizeof(mbox_path) -2)) {
			pin_set_err(ebufp, PIN_ERRLOC_FM,
			    PIN_ERRCLASS_SYSTEM_DETERMINATE,
