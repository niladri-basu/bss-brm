...
# @(#)$Id: default_values_75ps9.pl /cgbubrm_7.5.0.rwsmod/1 2014/05/15 04:31:14 mkothari Exp $ 
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps9/oracle/default_values_75ps9.pl /cgbubrm_7.5.0.rwsmod/1 2014/05/15 04:31:14 mkothari Exp $
#
# default_values_75ps9.pl
#
# Copyright (c) 2009, 2014, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      default_values_75ps9.pl -  This module will result in modyfying existing Business Params or
#                              doing Data Reorganization
#
#    DESCRIPTION
#      This File will contain subroutines to modifying existing Business Params or
#      doing Data Reorganization
#      This file will be called from pin_upgrade_75ps9.pl .
#
#    NOTES
#
#	This Script will be used to execute the script "default_values_75ps9.source"
#	The script "default_values_75ps9.source" will be containing the logic for
...
#	from this script.
#       The Driver File for upgrade which is the script "pin_upgrade_75ps9.pl" will call this
#       perl script (default_values_75ps9.pl) through one of its wrappers named "default_values_75ps9()"

#       This script will use the API "ExecuteShell_Piped" [which is a part of default installation]
#       to execute the contents in "default_values_75ps9.source". This API takes as
#       input the script "default_values_75ps9.source" file and will return the result based on the
...
#       be easier to track the error, while executing the default_values_75ps9.source script called from
#       the wrapper API [default_values_75ps9()] which in turn is called from the pin_upgrade_75ps9.pl.
...
our @DEFAULT_VALUES_SCRIPTS_75PS9 = ("bus_params_create_two_event.source");

sub default_values_75ps9 {

	my $file = "";
	my $time_stamp = "";
	my $tmpOutFile          = "$PIN_TEMP_DIR" . "/tmp_default_values75ps9.log";
	my $defaultvaluespinlog = "$PIN_TEMP_DIR" . "/default_values_75ps9.log";
	my $CMD = "";
	my $Result = 0;
	my $FirstElem = $DEFAULT_VALUES_SCRIPTS_75PS9[0];
	my $writestatement = "";
	my $return_status = -1;
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@DEFAULT_VALUES_SCRIPTS_75PS9) {
	...
	if ( $Result == 0 ) {
	        ...
	        open( TMPFILE3, ">>$defaultvaluespinlog " ) || die "cannot read $defaultvaluespinlog \n";
                $writestatement = "Successfull Execution of the function default_values_75ps9 \n";
                print TMPFILE3 ("$writestatement \n");
                close(TMPFILE3);
	} elsif ( $Result == 2 ) {
                print "File default_values_75ps9.source file is not found under $PIN_HOME/sys/dd/data directory. \n";
	} elsif ( $Result == 3 ) {
