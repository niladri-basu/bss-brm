...
#
# @(#)$Id: default_values_74ps1.pl /cgbubrm_7.3.2.rwsmod/1 2009/11/12 00:37:52 rasarka Exp $ 
#
# $Header: bus_platform_vob/upgrade/74_74ps1_upgrade/oracle/default_values_74ps1.pl /cgbubrm_7.3.2.rwsmod/1 2009/11/12 00:37:52 rasarka Exp $
#
# default_values_74ps1.pl
#
# Copyright (c) 2009, Oracle and/or its affiliates. All rights reserved. 
#
#    NAME
#      default_values_74ps1.pl -  This module will result in modyfying existing Business Params or
#                              doing Data Reorganization
#
#    DESCRIPTION
#      This File will contain subroutines to modifying existing Business Params or
#      doing Data Reorganization
#      This file will be called from pin_74_74ps1_upgrade.pl third in the order.
#
#    NOTES
#
#	This Script will be used to execute the script "default_values_74ps1.source"
#	The script "default_values_74ps1.source" will be containing the logic for
#	data reorganization of all 7.4ps1 Features having upgrade impact and also modification of the
#	existing business params. This is the only file which will be called
#	from this script.
#       The Driver File for upgrade which is the script "pin_74_74ps1_upgrade.pl" will call this
#       perl script (default_values_74ps1.pl) through one of its wrappers named "default_values_74ps1()"

#       This script will use the API "ExecuteShell_Piped" [which is a part of default installation]
#       to execute the contents in "default_values_74ps1.source". This API takes as
#       input the script "default_values_74ps1.source" file and will return the result based on the
...
#       be easier to track the error, while executing the default_values_74ps1.source script called from
#       the wrapper API [default_values_74ps1()] which in turn is called from the pin_74_74ps1_upgrade.pl.
...
our @DEFAULT_VALUES_SCRIPTS = ("default_values_74ps1.source");

sub default_values_74ps1 {

	my $file = "";
	my $time_stamp = "";
	my $tmpOutFile          = "$PIN_TEMP_DIR" . "/tmp_default_values74ps1.log";
	my $defaultvaluespinlog = "$PIN_TEMP_DIR" . "/default_values_74ps1.log";
	my $CMD = "";
	my $Result = 0;
	my $FirstElem = $DEFAULT_VALUES_SCRIPTS[0];
	my $writestatement = "";
	my $return_status = -1;
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@DEFAULT_VALUES_SCRIPTS) {
	...
	if ( $Result == 0 ) {
	        ...
	        open( TMPFILE3, ">>$defaultvaluespinlog " ) || die "cannot read $defaultvaluespinlog \n";
                $writestatement = "Successfull Execution of the function default_values_74ps1 \n";
                print TMPFILE3 ("$writestatement \n");
                close(TMPFILE3);
	} elsif ( $Result == 2 ) {
                print "File default_values_74ps1.source file is not found under $PIN_HOME/sys/dd/data directory. \n";
	} elsif ( $Result == 3 ) {
