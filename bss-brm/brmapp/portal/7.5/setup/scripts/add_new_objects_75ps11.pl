...
#
# @(#)$Id: add_new_objects_75ps11.pl /cgbubrm_7.5.0.rwsmod/3 2014/12/14 13:00:08 mkothari Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps11/oracle/add_new_objects_75ps11.pl /cgbubrm_7.5.0.rwsmod/3 2014/12/14 13:00:08 mkothari Exp $
#
# add_new_objects_75ps11.pl
#
# Copyright (c) 2008, 2014, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      add_new_objects_75ps11.pl - This module will result in creating new objects or modifying existing ones
#
#    DESCRIPTION
#      This File will contain subroutines to create objects or modify existing object behaviour
#      This file will be called from pin_75ps11_upgrade.pl first in the order.
...
#      add_new_objects_75ps11.pl for execution and updating the DB Schema
...
#      The Driver File for upgrade which is the script "pin_75ps11_upgrade.pl"
#      will call this perl script (add_new_objects_75ps11.pl) through one of its wrappers APIs
...
#       the driver file "pin_75ps11_upgrade.pl". So in case a failed result value is returned,
...
our @ALL_CREATE_SCRIPTS_PS11 = ("dd_objects_event_billing_adjustment_bill.source",
				"dd_objects_event_billing_dispute_bill.source",
				"dd_objects_event_billing_settlement_bill.source",
				"dd_objects_newsfeed_75ps11.source");

our @ALL_MODIFY_SCRIPTS_PS11 = ("dd_objects_event_customer_billinfo.source");
			

our $UPG_DIR   = "$DD{'location'}";
our $StartTime = "";

sub create_new_objects_75ps11 {
	my $file         = "";
	my $objectname    = "";
	my $time_stamp   = "";
	my $Result       = 0;
	...
	my $FirstElem           = $ALL_CREATE_SCRIPTS_PS11[0];
	my $createobjectspinlog = "$PIN_TEMP_DIR" . "/create_objects_75ps11.log";
	my $writestatement      = "";
        my $sqlstatement        = "";
        my $initobjres          =  1;
        my $ReadString          = "";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@ALL_CREATE_SCRIPTS_PS11) {
			...
			if ( !-e "$UPG_DIR/$file" ) {
				...
				if ( $file eq "dd_objects_event_billing_dispute_bill.source")
                                {
                                	$objectname = "/event/billing/dispute/bill";
                                }
				elsif ( $file eq "dd_objects_event_billing_adjustment_bill.source")
				{
					$objectname = "/event/billing/adjustment/bill";
				}	
				elsif ( $file eq "dd_objects_event_billing_settlement_bill.source")
				{
					$objectname = "/event/billing/settlement/bill";
				}	
				elsif ( $file eq "dd_objects_newsfeed_75ps11.source")
				{
					$objectname = "/newsfeed";
				}	
                                else
...
sub modify_existing_objects_75ps11 {
	...
	my $logfile =
	    ...
	    . "$MAIN_DM{'db'}->{'vendor'}" . "/" . "dm_"
	    . "$MAIN_DM{'db'}->{'vendor'}.pinlog";
	my $FirstElem           = $ALL_MODIFY_SCRIPTS_PS11[0];
	my $modifyobjectspinlog = "$PIN_TEMP_DIR" . "/modify_objects_75ps11.log";
	my $writestatement      = "";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file_mod (@ALL_MODIFY_SCRIPTS_PS11) 
		{
			...
			if ( !-e "$UPG_DIR/$file_mod" ) {
				...
				if ( $file_mod eq "dd_objects_event_customer_billinfo.source" )
                                {
					$tablename  = "event_customer_billinfo_t";
                                        $columnname = "actg_type";
