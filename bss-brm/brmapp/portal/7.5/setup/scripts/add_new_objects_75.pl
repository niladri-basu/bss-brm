...
#
# @(#)$Id: add_new_objects_75.pl /cgbubrm_7.5.0.rwsmod/5 2013/08/30 04:59:40 mkothari Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_chronos/oracle/add_new_objects_75.pl /cgbubrm_7.5.0.rwsmod/5 2013/08/30 04:59:40 mkothari Exp $
#
# add_new_objects_75.pl
#
# Copyright (c) 2008, 2013, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      add_new_objects_75.pl - This module will result in creating new objects or modifying existing ones
#
#    DESCRIPTION
#      This File will contain subroutines to create objects or modify existing object behaviour
#      This file will be called from pin_74_75_upgrade.pl first in the order.
...
#      add_new_objects_75.pl for execution and updating the DB Schema
...
#      The Driver File for upgrade which is the script "pin_74_75_upgrade.pl"
#      will call this perl script (add_new_objects_75.pl) through one of its wrappers APIs
...
#       the driver file "pin_74_75_upgrade.pl". So in case a failed result value is returned,
...
our @ALL_CREATE_SCRIPTS = ( "74_PDC_init_objects.source",
                            "dd_objects_event_corrective_bill.source",
                            "dd_objects_history_bills.source",
			    "dd_ass_bus_profile_templates.source",
			    "new_data_sequence_for_corr_inv.source");

our @ALL_MODIFY_SCRIPTS = ( "new_item_field_for_corrective_invoicing.source",
                            "dd_objects_event_def_taxes.source",
                            "dd_objects_event.source",
			    "74_PDC_create_plan_list_code.source",
                            "74_PDC_modify_deal.source",
			    "74_PDC_modify_plan.source",
			    "74_PDC_modify_product.source",
                            "74_PDC_modify_rate_plan.source",
                            "74_PDC_modify_rate_plan_selector.source",
			    "74_PDC_modify_rollover.source",
                            "74_PDC_modify_sponsorship.source",
			    "config_object_mod_75_tcf_aaa.source");
...
sub create_new_objects {
	...
	my $FirstElem           = $ALL_CREATE_SCRIPTS[0];
	my $createobjectspinlog = "$PIN_TEMP_DIR" . "/create_objects_75.log";
	my $writestatement      = "";
        my $sqlstatement        = "";
        my $initobjres          =  1;
        my $statusflags         = "TRUE";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@ALL_CREATE_SCRIPTS) {
			...
			if ( !-e "$UPG_DIR/$file" ) {
				...
				if ( $file eq "74_PDC_init_objects.source" )

				{
                                	$sqlstatement = "select name from config_t where name like 'pricing'; \n";
                                	...
                                	while( $ReadString = <TMPFILE> ) 
					{
                                                 ...
                                                 else 
						 {
                                ...
                                if ( $file eq "new_data_sequence_for_corr_inv.source" )
                                {
                                        $initobjres = 1;
                                        $sqlstatement = "select name from data_t where name like 'PIN_SEQUENCE_TYPE_CORR_BILL_NO'; \n";
                                        &ExecuteSQL_Statement( "$sqlstatement", $statusflags, $statusflags, %{$DM{"db"}});
                                        open(TMPFILE, "<$SETUP_LOG_FILENAME " ) || die "cannot read $SETUP_LOG_FILENAME \n";
                                        while( $ReadString = <TMPFILE> )
                                        {
                                        ...
                                        if ($initobjres == 0)
                                        {
                                         ...
                                         {
                                                print "Data sequence already exists. \n";
                                                open( TMPFILE3, ">>$createobjectspinlog " ) || die "cannot read $createobjectspinlog \n";
                                                $writestatement = "Data sequence already exists. \n";
                                                print TMPFILE3 ("$writestatement \n") ;
                                                close(TMPFILE3);
                                                print "################################################ \n";
                                                next;
                                ...
                                if ( $file eq "dd_objects_event_corrective_bill.source")
				{
					$tablename = "event_corrective_bill_t";
				} 
                                elsif ( $file eq "dd_objects_history_bills.source" )
				{
					$tablename = "history_bills_t";
  				}
                                elsif ( $file eq "create_slcm_tables_in_dd.source" )
                                {
                                       $tablename = "active_session_thresholds_t";
                                }
				elsif ( $file eq "dd_ass_bus_profile_templates.source" )
				{
					$tablename = "ass_bus_profile_templates_t";
				}
				else
				{
					print "Base table does not exist for $file , moving ahead\n";
					next;
				}

                                if ( VerifyPresenceOfObject ( $classname, %{ $DM{"db"} }) == 0 && $GlobalObject == 1)
				{
                                ...
                                elsif ( VerifyPresenceOfTable ( $tablename, %{ $DM{"db"} }) == 0)
                                {
                                        print "Creating Objects in BRM Database \n";
                                        print "Updating DB Schema\n";
                                        $SKIP_INIT_OBJECTS   = "YES";
...
sub modify_existing_objects {
	...
	my $logfile =
	    ...
	    . "$MAIN_DM{'db'}->{'vendor'}" . "/" . "dm_"
	    . "$MAIN_DM{'db'}->{'vendor'}.pinlog";
	my $FirstElem           = $ALL_MODIFY_SCRIPTS[0];
	my $modifyobjectspinlog = "$PIN_TEMP_DIR" . "/modify_objects_75.log";
	my $writestatement      = "";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file_mod (@ALL_MODIFY_SCRIPTS) 
		{
			...
			if ( !-e "$UPG_DIR/$file_mod" ) {
				...
				$GlobalObject = 0;
                                ...
                                if ( $file_mod eq "74_PDC_create_plan_list_code.source")
                                {
                                        $classname = "/group/plan_list";
					$columnname = "PIN_FLD_PLAN_LIST_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
                                } elsif ( $file_mod eq "config_object_mod_75_tcf_aaa.source" && VerifyPresenceOfObject ( "/config/aaa", %{ $DM{"db"} }) != 0)
				{
					$classname = "/config/aaa";
					$columnname = "PIN_FLD_SCALED_DELAY_INFO";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_deal.source" )
				{
					$classname  = "/deal";
					$columnname = "PIN_FLD_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_plan.source" )
				{
					$classname  = "/plan";
					$columnname = "PIN_FLD_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_product.source")
				{
					$classname  = "/product";
					$columnname = "PIN_FLD_RATE_PLAN_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_rate_plan.source")
				{
					$classname  = "/rate_plan";
					$columnname = "PIN_FLD_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_rate_plan_selector.source")
				{
					$classname  = "/rate_plan_selector";
					$columnname = "PIN_FLD_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_rollover.source" )
				{
					$classname = "/rollover";
					$columnname = "PIN_FLD_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
				} elsif ( $file_mod eq "74_PDC_modify_sponsorship.source" )
				{
					$classname  = "/sponsorship";
					$columnname = "PIN_FLD_CODE";
					$column_exist_flag = "FALSE";
					$GlobalObject = 1;
                                } elsif ( $file_mod eq "new_item_field_for_corrective_invoicing.source" )
				{
					$tablename  = "ITEM_T";
                                        $columnname = "FLAGS";
					$column_exist_flag = "FALSE";
...
sub optional_managers_schema_creation_upgrade {
        my $file_mod   = "";
        my $tablename  = "";
        ...
        my $optionalmanagercreateobjlog = "$PIN_TEMP_DIR" . "/optional_manager_objects_creation_74.log";
        my $writestatement      = "";
...
sub optional_managers_schema_upgrade {
	my $file_mod   = "";
        my $tablename  = "";
        my $Result     = 0;
        my $logfile =
