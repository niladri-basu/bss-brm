...
#
# @(#)$Id: add_new_objects_75ps7.pl /cgbubrm_7.5.0.rwsmod/2 2014/01/12 21:51:28 mkothari Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps7/oracle/add_new_objects_75ps7.pl /cgbubrm_7.5.0.rwsmod/2 2014/01/12 21:51:28 mkothari Exp $
#
# add_new_objects_75ps7.pl
#
# Copyright (c) 2008, 2014, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      add_new_objects_75ps7.pl - This module will result in creating new objects or modifying existing ones
#
#    DESCRIPTION
#      This File will contain subroutines to create objects or modify existing object behaviour
#      This file will be called from pin_75ps7_upgrade.pl first in the order.
...
#      add_new_objects_75ps7.pl for execution and updating the DB Schema
...
#      The Driver File for upgrade which is the script "pin_75ps7_upgrade.pl"
#      will call this perl script (add_new_objects_75ps7.pl) through one of its wrappers APIs
...
#       the driver file "pin_75ps7_upgrade.pl". So in case a failed result value is returned,
...
our @ALL_CREATE_SCRIPTS_PS7 = (
);

our @ALL_MODIFY_SCRIPTS_PS7 = ( "dd_object_ca_discount_inactivation_1.source",
				"dd_object_ca_discount_inactivation_2.source",
				"dd_objects_failed_bill_obj.source");
			

our $UPG_DIR   = "$DD{'location'}";
our $StartTime = "";

sub create_new_objects_75ps7 {
	my $file         = "";
	my $tablename    = "";
	my $time_stamp   = "";
	my $Result       = 0;
	...
	my $FirstElem           = $ALL_CREATE_SCRIPTS_PS7[0];
	my $createobjectspinlog = "$PIN_TEMP_DIR" . "/create_objects_75ps7.log";
	my $writestatement      = "";
        my $sqlstatement        = "";
        my $initobjres          =  1;
        my $statusflags         = "TRUE";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@ALL_CREATE_SCRIPTS_PS7) {
...
sub modify_existing_objects_75ps7 {
	...
	my $logfile =
	    ...
	    . "$MAIN_DM{'db'}->{'vendor'}" . "/" . "dm_"
	    . "$MAIN_DM{'db'}->{'vendor'}.pinlog";
	my $FirstElem           = $ALL_MODIFY_SCRIPTS_PS7[0];
	my $modifyobjectspinlog = "$PIN_TEMP_DIR" . "/modify_objects_75ps7.log";
	my $writestatement      = "";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file_mod (@ALL_MODIFY_SCRIPTS_PS7) 
		{
			...
			if ( !-e "$UPG_DIR/$file_mod" ) {
				...
				print "$time_stamp \n";
                                ...
                                if ( $file_mod eq "dd_object_ca_discount_inactivation_1.source" )
                                {
                                        $tablename  = "purchased_discount_t";
                                        $columnname = "str_val";
                                        $column_exist_flag = "FALSE";
                                }
				elsif ( $file_mod eq "dd_object_ca_discount_inactivation_2.source" )
                                {
                                        $tablename  = "event_billing_disc_action_t";
                                        $columnname = "str_val";
                                        $column_exist_flag = "FALSE";
                                }
                                elsif ( $file_mod eq "dd_objects_failed_bill_obj.source" && VerifyPresenceOfTable ("proc_aud_bill_err_acct_t" ,%{ $DM{"db"} }) != 0)
                                {
                                        $tablename  = "proc_aud_bill_err_acct_t";
                                        $columnname = "BILL_OBJ_DB";
