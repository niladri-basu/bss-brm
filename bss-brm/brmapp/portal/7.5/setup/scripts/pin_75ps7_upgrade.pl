...
#
# @(#)$Id: pin_75ps7_upgrade.pl /cgbubrm_7.5.0.rwsmod/3 2013/12/04 01:13:25 mkothari Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps7/oracle/pin_75ps7_upgrade.pl /cgbubrm_7.5.0.rwsmod/3 2013/12/04 01:13:25 mkothari Exp $
#
# pin_75ps7_upgrade.pl
# 
# Copyright (c) 2009, 2013, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      pin_75ps7_upgrade.pl - this script is the main driver file
#      for upgrading the BRM PORTALBASE DB to 7.5 PS7
#
#    DESCRIPTION
...
our $upgrade75pinlog = "$PIN_TEMP_DIR" . "/" . "upgrade_75ps7.log";
our $msgstring = "";
our $return_status  = 0;
our $writestatement = "";
...
if ( $ret_val == 0 || $ret_val == -1) {
	...
	if (VerifyPresenceOfTable($collection_opt_mgr_table, %{ $DM{"db"} }) == 0) {
        ...
        print "PortalBase Schema upgrade to BRM 75ps7 is successful. \n";
        open(UPGRADE_PINLOG_FILE_HANDLER, ">>$upgrade75pinlog " ) || die "cannot open $upgrade75pinlog for writing. \n";
        $msgstring ="PortalBase Schema upgrade to BRM 75ps7 is successful. \n";
...
sub upgrade_75 {
        ...
        my $api_result = -1;
        my $Result = 0;
        my $upgrade75pinlog = "$PIN_TEMP_DIR" . "/" . "upgrade_75ps7.log";
        my $writestatement = "";
	...
	# cleaning up the upgrade log file before executing upgrade scripts.
        ...
        if ( $firstelem eq "" ) {
                ...
                foreach $upgrade_api (@EXECUTE_ALL_UPGRADE_SCRIPTS) {
                                print "################################################ \n";
                                print "Upgrade BRM 75ps7 is now executing $upgrade_api \n";
