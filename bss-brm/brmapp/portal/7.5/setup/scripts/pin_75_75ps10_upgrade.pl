...
#
# @(#)$Id: pin_75_75ps10_upgrade.pl /cgbubrm_7.5.0.rwsmod/4 2014/09/09 02:44:55 mkothari Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps10/oracle/pin_75_75ps10_upgrade.pl /cgbubrm_7.5.0.rwsmod/4 2014/09/09 02:44:55 mkothari Exp $
#
...
#
#    NAME
#      pin_75_75ps10_upgrade.pl - this script is the main driver file
#      for upgrading the BRM PORTALBASE DB to 7.5 PS10
#
#    DESCRIPTION
#	This script is the main driver file for upgrading the BRM Database to 7.5 PS10
...
#
#       e.g. In the Driver file "pin_75_75ps10_upgrade.pl" there will be a subroutine like
...
require "add_new_objects_75ps7.pl";
require "default_values_75ps7.pl";
require "add_new_objects_75ps8.pl";
require "default_values_75ps8.pl";
require "add_new_objects_75ps9.pl";
require "default_values_75ps9.pl";
require "add_new_objects_75ps10.pl";
require "default_values_75ps10.pl";
...
our $upgrade75pinlog = "$PIN_TEMP_DIR" . "/" . "upgrade_75ps10.log";
our $msgstring = "";
our $return_status  = 0;
our $writestatement = "";
...
our $statusflag = "TRUE";
our $SEPA = '$PIN_HOME/apps/pin_sepa';
...
our @EXECUTE_ALL_UPGRADE_SCRIPTS = (
	...
	"modify_existing_objects_75ps7()",
	"default_values_75ps7()",
	"create_new_objects_75ps8()",
	"modify_existing_objects_75ps8()",
	"default_values_75ps8()",
	"create_new_objects_75ps9()",
	"modify_existing_objects_75ps9()",
	"default_values_75ps9()",
	"create_new_objects_75ps10()",
	"modify_existing_objects_75ps10()",
	"default_values_75ps10()"
        );
...
# Empty Directories for Sepa Project
        system("mkdir -p $SEPA/sepa_dd ; mkdir -p $SEPA/sepa_ct ; mkdir -p $SEPA/sepa_rev ; mkdir -p $SEPA/sepa_resp/done ; mkdir -p $SEPA/sepa_resp/error ; mkdir -p $SEPA/sepa_resp/input");
        chmod( 0755, $SEPA);
...
if ( $ret_val == 0 || $ret_val == -1) {
	...
	if (VerifyPresenceOfTable($collection_opt_mgr_table, %{ $DM{"db"} }) == 0) {
        ...
        print "PortalBase Schema upgrade to BRM 75ps10 is successful. \n";
        open(UPGRADE_PINLOG_FILE_HANDLER, ">>$upgrade75pinlog " ) || die "cannot open $upgrade75pinlog for writing. \n";
        $msgstring ="PortalBase Schema upgrade to BRM 75ps10 is successful. \n";
	...
	# Executing the perl script to generate utilities
	 if( -e "$PIN_HOME/setup/scripts/pin_install_upgrade.pl" )
        {
                print "Executing $PIN_HOME/setup/scripts/pin_install_upgrade.pl \n";
        	$msgstring =  "Executing $PIN_HOME/setup/scripts/pin_install_upgrade.pl \n";
        	print UPGRADE_PINLOG_FILE_HANDLER $msgstring;
                system("$PIN_HOME/setup/scripts/pin_install_upgrade.pl");
        }
...
sub load_utilities {
   ...
   eval "&load_localized_strings_note_en();";
   eval "&load_localized_strings_reasons_en();";
   chdir $CurrentDir;
...
sub upgrade_75 {
        ...
        my $api_result = -1;
        my $Result = 0;
        my $upgrade75pinlog = "$PIN_TEMP_DIR" . "/" . "upgrade_75ps10.log";
        my $writestatement = "";
	...
	# cleaning up the upgrade log file before executing upgrade scripts.
        ...
        if ( $firstelem eq "" ) {
                ...
                foreach $upgrade_api (@EXECUTE_ALL_UPGRADE_SCRIPTS) {
                                print "################################################ \n";
                                print "Upgrade BRM 75ps10 is now executing $upgrade_api \n";
