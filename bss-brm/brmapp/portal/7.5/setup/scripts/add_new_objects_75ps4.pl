...
#
# @(#)$Id: add_new_objects_75ps4.pl /cgbubrm_7.5.0.rwsmod/3 2013/02/22 03:30:40 mkothari Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps4/oracle/add_new_objects_75ps4.pl /cgbubrm_7.5.0.rwsmod/3 2013/02/22 03:30:40 mkothari Exp $
#
# add_new_objects_75ps4.pl
#
# Copyright (c) 2008, 2013, Oracle and/or its affiliates. All rights reserved. 
#
#    NAME
#      add_new_objects_75ps4.pl - This module will result in creating new objects or modifying existing ones
#
#    DESCRIPTION
#      This File will contain subroutines to create objects or modify existing object behaviour
#      This file will be called from pin_74_75ps4_upgrade.pl first in the order.
...
#      add_new_objects_75ps4.pl for execution and updating the DB Schema
...
#      The Driver File for upgrade which is the script "pin_74_75ps4_upgrade.pl"
#      will call this perl script (add_new_objects_75ps4.pl) through one of its wrappers APIs
...
#       the driver file "pin_74_75ps4_upgrade.pl". So in case a failed result value is returned,
...
our @ALL_CREATE_SCRIPTS_PS4 = ("dd_objects_collections_75ps4_upgrade.source");

our @ALL_MODIFY_SCRIPTS_PS4 = ("dd_objects_invoice_75ps4.source",
				"dd_objects_ipv6_75ps4_upgrade.source");
...
sub create_new_objects_75ps4 {
	...
	my $FirstElem           = $ALL_CREATE_SCRIPTS_PS4[0];
	my $createobjectspinlog = "$PIN_TEMP_DIR" . "/create_objects_75ps4.log";
	my $writestatement      = "";
        my $sqlstatement        = "";
        my $initobjres          =  1;
        my $statusflags         = "TRUE";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@ALL_CREATE_SCRIPTS_PS4) {
			...
			if ( !-e "$UPG_DIR/$file" ) {
				...
				if ( $file eq "dd_objects_collections_75ps4_upgrade.source" && VerifyPresenceOfObject ( "/collections_scenario", %{ $DM{"db"} }) != 0)
				{
					$tablename = "profile_collections_params_t";
				}
				else
                                {
                                        next;
...
sub modify_existing_objects_75ps4 {
	...
	my $logfile =
	    ...
	    . "$MAIN_DM{'db'}->{'vendor'}" . "/" . "dm_"
	    . "$MAIN_DM{'db'}->{'vendor'}.pinlog";
	my $FirstElem           = $ALL_MODIFY_SCRIPTS_PS4[0];
	my $modifyobjectspinlog = "$PIN_TEMP_DIR" . "/modify_objects_75ps4.log";
	my $writestatement      = "";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file_mod (@ALL_MODIFY_SCRIPTS_PS4) 
		{
			...
			if ( !-e "$UPG_DIR/$file_mod" ) {
				...
				if ( $file_mod eq "dd_objects_invoice_75ps4.source" )
				{
					$tablename  = "invoice_t";
					$columnname = "STATUS";
					$column_exist_flag = "TRUE";
				} 
				elsif ( $file_mod eq "dd_objects_ipv6_75ps4_upgrade.source" )
				{
					$tablename  = "event_t";
					$columnname = "provider_ipaddr";
					$column_exist_flag = "TRUE";
				} 
				else
				{
...
sub optional_managers_schema_creation_upgrade_75ps4 {
        my $file_mod   = "";
        my $tablename  = "";
...
sub optional_managers_schema_upgrade_75ps4 {
	my $file_mod   = "";
        my $tablename  = "";
        my $Result     = 0;
        my $logfile =
