...
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps10/oracle/default_values_75ps10.pl /cgbubrm_7.5.0.rwsmod/8 2014/12/14 13:00:08 mkothari Exp $
#
# default_values_75ps10.pl
#
# Copyright (c) 2009, 2014, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      default_values_75ps10.pl -  This module will result in modyfying existing Business Params or
#                              doing Data Reorganization
#
#    DESCRIPTION
#      This File will contain subroutines to modifying existing Business Params or
#      doing Data Reorganization
#      This file will be called from pin_upgrade_75ps10.pl .
#
#    NOTES
#
#	This Script will be used to execute the script "default_values_75ps10.source"
#	The script "default_values_75ps10.source" will be containing the logic for
...
#	from this script.
#       The Driver File for upgrade which is the script "pin_upgrade_75ps10.pl" will call this
#       perl script (default_values_75ps10.pl) through one of its wrappers named "default_values_75ps10()"

#       This script will use the API "ExecuteShell_Piped" [which is a part of default installation]
#       to execute the contents in "default_values_75ps10.source". This API takes as
#       input the script "default_values_75ps10.source" file and will return the result based on the
...
#       be easier to track the error, while executing the default_values_75ps10.source script called from
#       the wrapper API [default_values_75ps10()] which in turn is called from the pin_upgrade_75ps10.pl.
...
our @DEFAULT_VALUES_SCRIPTS_75PS10 = (  "7.4PS2_create_indexes_collections_AIA.source",
					"7.4PS2_create_collections_view_AIA.source",
					"bus_params_zero_event_balances.source",
					"bus_param_use_billed_time_for_gl.source",
					"bus_param_item_overallocation.source",
					"drop_object_newsfeed_75ps11.source");

sub default_values_75ps10 {

	my $file = "";
	my $time_stamp = "";
	my $tmpOutFile          = "$PIN_TEMP_DIR" . "/tmp_default_values75ps10.log";
	my $defaultvaluespinlog = "$PIN_TEMP_DIR" . "/default_values_75ps10.log";
	my $CMD = "";
	my $Result = 0;
	my $FirstElem = $DEFAULT_VALUES_SCRIPTS_75PS10[0];
	my $writestatement = "";
	my $return_status = -1;
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@DEFAULT_VALUES_SCRIPTS_75PS10) {
		...
		print "################################################ \n";
			...
			elsif ( -e "$UPG_APP_DIR/$file" )
			{
				print "filename is $UPG_APP_DIR/$file \n";
                        ...
                        {
                                print "file $UPG_DIR/$file not found \n";
				next;
                        }
				...
				&& ( $MAIN_DM{'db'}->{'vendor'} =~ /oracle/ ))
			{
				...
				while ( my $ReadString = <TMPFILE2> ) {
					# Check for ORA errors and compilation warnings
	...
	if ( $Result == 0 ) {
	        ...
	        open( TMPFILE3, ">>$defaultvaluespinlog " ) || die "cannot read $defaultvaluespinlog \n";
                $writestatement = "Successfull Execution of the function default_values_75ps10 \n";
                print TMPFILE3 ("$writestatement \n");
                close(TMPFILE3);
	} elsif ( $Result == 2 ) {
                print "File default_values_75ps10.source file is not found under $PIN_HOME/sys/dd/data directory. \n";
	} elsif ( $Result == 3 ) {
