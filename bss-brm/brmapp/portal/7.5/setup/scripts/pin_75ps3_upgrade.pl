...
#
# @(#)$Id: pin_75ps3_upgrade.pl /cgbubrm_7.5.0.rwsmod/1 2012/09/17 06:10:11 hsnagpal Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps3/oracle/pin_75ps3_upgrade.pl /cgbubrm_7.5.0.rwsmod/1 2012/09/17 06:10:11 hsnagpal Exp $
#
# pin_75ps3_upgrade.pl
# 
# Copyright (c) 2009, 2012, Oracle and/or its affiliates. All rights reserved. 
#
#    NAME
#      pin_75ps3_upgrade.pl - this script is the main driver file
#      for upgrading the BRM PORTALBASE DB to 7.5 
#
#    DESCRIPTION
#	This script is the main driver file for upgrading the BRM Database to 7.5 
...
our $upgrade75pinlog = "$PIN_TEMP_DIR" . "/" . "upgrade_75ps3.log";
our $msgstring = "";
our $return_status  = 0;
our $writestatement = "";
...
if ( $ret_val == 0 || $ret_val == -1) {
	...
	my $rel_opt_mgr_table = "BATCH_REL_T";
        ...
        my $procfiles = "";
	my $collection_opt_mgr_table = "CONFIG_COLLECTIONS_ACTION_T";
	...
	if (VerifyPresenceOfTable($collection_opt_mgr_table, %{ $DM{"db"} }) == 0) {
        ...
        print "PortalBase Schema upgrade to BRM 75ps3 is successful. \n";
        open(UPGRADE_PINLOG_FILE_HANDLER, ">>$upgrade75pinlog " ) || die "cannot open $upgrade75pinlog for writing. \n";
        $msgstring ="PortalBase Schema upgrade to BRM 75ps3 is successful. \n";
	...
	if( -e "$PIN_HOME/setup/scripts/pin_cmp_upgrade.pl" )
        ...
        # Calling application load utility
...
sub upgrade_75 {
        ...
        my $api_result = -1;
        my $Result = 0;
        my $upgrade75pinlog = "$PIN_TEMP_DIR" . "/" . "upgrade_75ps3.log";
        my $writestatement = "";
	...
	# cleaning up the upgrade log file before executing upgrade scripts.
        ...
        if ( $firstelem eq "" ) {
                ...
                foreach $upgrade_api (@EXECUTE_ALL_UPGRADE_SCRIPTS) {
                                print "################################################ \n";
                                print "Upgrade BRM 75ps3 is now executing $upgrade_api \n";
