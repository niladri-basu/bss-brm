...
#
# @(#)$Id: add_new_objects_75ps6.pl /cgbubrm_7.5.0.rwsmod/3 2013/12/15 22:42:27 samhegde Exp $
#
# $Header: bus_platform_vob/upgrade/upgrade_75ps6/oracle/add_new_objects_75ps6.pl /cgbubrm_7.5.0.rwsmod/3 2013/12/15 22:42:27 samhegde Exp $
#
# add_new_objects_75ps6.pl
#
# Copyright (c) 2008, 2013, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      add_new_objects_75ps6.pl - This module will result in creating new objects or modifying existing ones
#
#    DESCRIPTION
#      This File will contain subroutines to create objects or modify existing object behaviour
#      This file will be called from pin_75ps6_upgrade.pl first in the order.
...
#      add_new_objects_75ps6.pl for execution and updating the DB Schema
...
#      The Driver File for upgrade which is the script "pin_75ps6_upgrade.pl"
#      will call this perl script (add_new_objects_75ps6.pl) through one of its wrappers APIs
...
#       the driver file "pin_75ps6_upgrade.pl". So in case a failed result value is returned,
...
our @ALL_CREATE_SCRIPTS_PS6 = (
);

our @ALL_MODIFY_SCRIPTS_PS6 = ( "dd_objects_cross_schema_transfer_upgrade.source",
				"dd_objects_cross_schema_transfer_upgrade_2.source",
				"dd_objects_cross_schema_transfer_upgrade_3.source",
				"dd_objects_audit_coll_gdtls_name_length.source",
				"dd_objects_service_passwd_length.source");
...
sub create_new_objects_75ps6 {
	...
	my $FirstElem           = $ALL_CREATE_SCRIPTS_PS6[0];
	my $createobjectspinlog = "$PIN_TEMP_DIR" . "/create_objects_75ps6.log";
	my $writestatement      = "";
        my $sqlstatement        = "";
        my $initobjres          =  1;
        my $statusflags         = "TRUE";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file (@ALL_CREATE_SCRIPTS_PS6) {
...
sub modify_existing_objects_75ps6 {
	...
	my $logfile =
	    ...
	    . "$MAIN_DM{'db'}->{'vendor'}" . "/" . "dm_"
	    . "$MAIN_DM{'db'}->{'vendor'}.pinlog";
	my $FirstElem           = $ALL_MODIFY_SCRIPTS_PS6[0];
	my $modifyobjectspinlog = "$PIN_TEMP_DIR" . "/modify_objects_75ps6.log";
	my $writestatement      = "";
	...
	if ( $FirstElem eq "" ) {
		...
		foreach $file_mod (@ALL_MODIFY_SCRIPTS_PS6) 
		{
			...
			if ( !-e "$UPG_DIR/$file_mod" ) {
				...
				print "$time_stamp \n";
                                ...
                                if ( $file_mod eq "dd_objects_cross_schema_transfer_upgrade.source" )
                                {
                                        $tablename  = "event_audit_services_t";
                                        $columnname = "target_service_obj_db";
                                        $column_exist_flag = "FALSE";
                                }
				elsif ( $file_mod eq "dd_objects_cross_schema_transfer_upgrade_2.source" )
                                {
                                        $tablename  = "event_audit_svc_bg_transfer_t";
                                        $columnname = "target_service_obj_db";
                                        $column_exist_flag = "FALSE";
                                }
                                elsif ( $file_mod eq "dd_objects_cross_schema_transfer_upgrade_3.source" )
                                {
                                        $tablename  = "event_audit_devices_t";
                                        $columnname = "target_service_obj_db";
                                        $column_exist_flag = "FALSE";
                                }
                                elsif ( $file_mod eq "dd_objects_audit_coll_gdtls_name_length.source" && VerifyPresenceOfTable("collections_action_t",%{ $DM{"db"} }) != 0)
                                {
                                        $tablename  = "event_audit_coll_gdtls_t";
                                        $columnname = "name";
                                        $column_exist_flag = "TRUE";
                                }
                                elsif ( $file_mod eq "dd_objects_service_passwd_length.source" )
                                {
                                        $tablename  = "service_t";
                                        $columnname = "passwd";
                                        $column_exist_flag = "TRUE";
...
sub optional_managers_schema_creation_upgrade_75ps6 {
        my $file_mod   = "";
        my $tablename  = "";
...
sub optional_managers_schema_upgrade_75ps6 {
	my $file_mod   = "";
        my $tablename  = "";
        my $Result     = 0;
        my $logfile =
