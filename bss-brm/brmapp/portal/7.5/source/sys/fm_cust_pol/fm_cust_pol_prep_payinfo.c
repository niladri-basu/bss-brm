 ...
 *	Change history
 *	Date: 09Apr2015		Modified by Dev Sharma
 *				to handle custom sub-class /payinfo/prepaid
 *******************************************************************/

#ifndef lint
static  char Sccs_Id[] = "@(#)$Id: fm_cust_pol_prep_payinfo.c /cgbubrm_7.5.0.rwsmod/2 2013/09/12 11:29:28 amandavi Exp $";
#endif
	...
	*ret_flistpp = NULL;
	...
	if (PIN_ERR_IS_ERR(ebufp)) {
		*ret_flistpp = (pin_flist_t *)NULL;
		...
		/* Defect 58: Customization by Dev Sharma on 12Nov2014*/
		// Start change

		poidp = (poid_t *)PIN_FLIST_FLD_GET(r_flistp, PIN_FLD_POID, 0, ebufp);

        	pay_type = PIN_POID_GET_TYPE(poidp);

        	if (pay_type && !strcmp(pay_type, PIN_OBJ_TYPE_PAYINFO_DD)) {

                	dd_flistp = PIN_FLIST_COPY(r_flistp,ebufp);
                	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,"op_cust_pol_prep_payinfo..copied",dd_flistp);

                	dd_info_flistp = PIN_FLIST_ELEM_TAKE(dd_flistp, PIN_FLD_DD_INFO,PIN_ELEMID_ANY, 1 , ebufp);
		
			if (PIN_FLIST_FLD_GET(dd_info_flistp, PIN_FLD_TYPE,1,ebufp) == NULL)
			{
				PIN_FLIST_FLD_SET(dd_info_flistp, PIN_FLD_TYPE, (void *)&type, ebufp);
			}

                	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                                "op_cust_pol_prep_payinfo output flist...Defect", dd_info_flistp);

                	tmp_flistp = PIN_FLIST_ELEM_ADD(dd_flistp, PIN_FLD_DD_INFO, 0, ebufp);

                	PIN_FLIST_CONCAT(tmp_flistp, dd_info_flistp, ebufp);

        		r_flistp = PIN_FLIST_COPY(dd_flistp,ebufp);

	
			/*Release Memory*/	
			PIN_FLIST_DESTROY_EX(&dd_flistp,ebufp);
			PIN_FLIST_DESTROY_EX(&dd_info_flistp,ebufp);
	...
	pin_errbuf_t		*ebufp)
{
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                        "op_cust_pol_prep_payinfo TEST_1 flist", in_flistp);
	...
	if( type != (u_int*)NULL ){
			...
			field = PIN_FLD_DDEBIT_INFO;
                	break;
		case PIN_PAY_TYPE_PREPAID: //9Apr2015: Added by Dev Sharma to handle custom sub-class /payinfo/prepaid
                        b_flistp = PIN_FLIST_ELEM_GET(i_flistp, PIN_FLD_CASH_INFO,
                                        PIN_ELEMID_ANY,1,ebufp);
                        field = PIN_FLD_CASH_INFO;
                        break;
		case PIN_PAY_TYPE_FARMSOURCE: //19052022: to handle custom sub-class /payinfo/farmsource
                        b_flistp = PIN_FLIST_ELEM_GET(i_flistp, PIN_FLD_FS_INFO,
                                        PIN_ELEMID_ANY,1,ebufp);
                        field = PIN_FLD_FS_INFO;
                        break;
		...
		switch( field ) {
			...
			case PIN_FLD_DDEBIT_INFO:
				...
				break;
			case PIN_FLD_CASH_INFO:	//9Apr2015: Added by Dev Sharma to handle custom sub-class /payinfo/prepaid
				err_msg = "PIN_FLD_CASH_INFO is missing";
				break;
			case PIN_FLD_FS_INFO: //to handle custom sub-class /payinfo/farmsource
				err_msg = "PIN_FLD_FS_INFO is missing";
                                break;
		}
		if (err_msg) {
			pin_set_err(ebufp, PIN_ERRLOC_FM, 
  ...
  "Unable to read payinfo default invoice_db poid from pin.conf",ebufp);
	...
	if (PIN_POID_GET_ID(pdp) == -1)  {	
		...
		if (pay_type == (u_int *)NULL) {
			...
			case PIN_PAY_TYPE_SMARTC:
			 ...
			 case PIN_PAY_TYPE_FARMSOURCE:
                                strcpy(poid_type, "/payinfo/farmsource");
                                option = 1;
                                break;
	 ...
	 ***********************************************************/
        ...
        /***********************************************************
         * 9Apr2015: Added by Dev Sharma to handle custom 
	 * /payinfo/prepaid sub-class
         * PREPAID PAYMENT TYPE 
         *
         ***********************************************************/


	} else if (!strcmp(poid_type, "/payinfo/prepaid")) {
                vp = PIN_FLIST_ELEM_GET(flistp, PIN_FLD_CASH_INFO,
                        PIN_ELEMID_ANY, option, ebufp);
   
	        if (vp != NULL) {

                        PIN_FLIST_ELEM_SET(*out_flistpp, vp,
                                        PIN_FLD_CASH_INFO, 0, ebufp);
                }

	 }
        /* /payinfo/farmsource sub-class farmsource ***/

	else if (!strcmp(poid_type, "/payinfo/farmsource")) {
                vp = PIN_FLIST_ELEM_GET(flistp, PIN_FLD_FS_INFO,
                        PIN_ELEMID_ANY, option, ebufp);

                if (vp != NULL) {

                        PIN_FLIST_ELEM_SET(*out_flistpp, vp,
                                        PIN_FLD_FS_INFO, 0, ebufp);
                }
	 ...
	 ***********************************************************/
		...
		case 4:
			...
			if ((cp[1] < '0') || (cp[1] > '9')) {
				...
				pin_snprintf(exp, sizeof(exp), "%02s%02s", month, year);
			} else {
		...
		case 6:
			...
			pin_snprintf(exp, sizeof(exp), "%02s%02s", month, (char *)&year[2]);
			break;
		case 7:
			/*
