	...
	*out_flistpp = NULL;
	...
	while ((flistp = PIN_FLIST_ELEM_GET_NEXT(in_flistp, PIN_FLD_NAMEINFO,
		...
		if( tax_valid_flag && (elemid == PIN_NAMEINFO_BILLING) &&
			...
			do_tax_validation(ctxp, in_flistp,elemid, &s_flistp, ebufp);
		}
...
continue_loop:
		...
		if (PIN_FLIST_ELEM_COUNT(s_flistp, PIN_FLD_FIELD, ebufp) > 0)
		{
			...
			if(elemid == PIN_NAMEINFO_BILLING){
	...
	pin_errbuf_t	*ebufp)
{
	...
	/* In case of GeoCode based Taxation and for the billing
	 * nameinfo , geocode returned by third party software
	 * needs to be captured here.In case subscriber has given any
	 * geocode, it will be compared with the geocodes returned
	 */
	if (bparam_location_mode_for_taxation == PIN_RATE_LOC_GEOCODE && elemid == PIN_NAMEINFO_BILLING){

		flistp = PIN_FLIST_ELEM_GET(in_flistp, PIN_FLD_NAMEINFO,
