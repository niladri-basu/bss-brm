...
{ 
	...
	`cd $BIP_INPUT_DIR`;
	`cd $BIP_INPUT_DIR;./docgen_trial.sh InvoiceList.xml > $trial_bipout_file`;
	`cd -`;
	#`./docgen_trial.sh  > $trial_bipout_file`;
#	`sh $BIP_INPUT_DIR/docgen_trial.sh  > $trial_bipout_file`;
...
print scalar localtime()."\n";
system("sh ./all_scripts/td_prebill_report.sh $dom_date");
	if ($? == 256) {
    	print "Exit\n";
    	exit;
	}
