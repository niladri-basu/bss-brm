...
echo "#################################################################"
echo "$SCRIPT_NAME started at "`date`  >> /brmapp/portal/7.5/TD/apps/td_farmsource_billing/td_fs_billing_crontab.log
echo "#################################################################"
echo

cd $PIN_HOME/TD/apps/td_farmsource_billing
echo "I am at $PIN_HOME/TD/apps/td_farmsource_billing"
...
if [ "$#" -eq  "0" ]
   then
     ...
     ./billing_file_control.pl -i $2  >> /brmapp/portal/7.5/TD/apps/td_farmsource_billing/td_fs_billing_crontab.log
fi

echo "End of farmsource_billing"
echo
echo "#################################################################"
