...
FETCH_PRODUCT_OUT_BACKUP_DIR=`egrep -v "#" login.cfg | grep "FETCH_PRODUCT_OUT_BACKUP_DIR" |cut -d'=' -f2`
FETCH_PRODUCT_OUT_DIR=`egrep -v "#" login.cfg | grep "FETCH_PRODUCT_OUT_DIR" |cut -d'=' -f2`
#ARCHIVE_DIR="$LOG_DIR/ARCHIVE_DIR";

ARCHIVE_DIR=`egrep -v "#" login.cfg | grep "ARCHIVE_DIR" |cut -d'=' -f2`

#Name of the script
SCRIPT_NAME="td_product_output_formatting.sh"
...
echo "#################################################################" >> $LOG_DIR/td_fetch_gl_details_extract.log
			...
			#cd `$FETCH_PRODUCT_OUT_DIR`
			fileval=`ls -l $FETCH_PRODUCT_OUT_DIR |wc -l`
			#echo "$fileval"
			if [ $fileval -ge 1 ]; then			
				for file1 in $FETCH_PRODUCT_OUT_DIR/*; do
					if [ -f "$file1" ]; then 
						filename=${file1##*/}
						...
						#echo $lines
						#now=`date +'%Y%m%d%H'`
						target_file=$EXT_OUT_DIR/BRM_PLAN_PRODUCT_EXPORT-$date_run.txt
						`cp $file1 $target_file`
						#`sed -i 'Total number of rows = ' $target_file`
						echo "Record_count*$count" >>$target_file
						sed -i '1 s/^/ACCOUNT_NO*BILL_INFO_ID*BILL_CYCLE*MSISDN*PRODUCT_NAME*PRICE*PRD_START_DATE*PP_STATUS*PRODUCT_END*LAST_BILL_DATE*NEXT_BILL_DATE*GL_ID*ACCOUNT_POID*BILL_INFO_POID*PURCHASE_PRD_POID*SERVICE_POID*PRODUCT_POID*PRD_NODE_LOCATION\n/' $target_file
						echo "rows" >> $LOG_DIR/td_fetch_gl_details_extract.log
					fi 
				done
			fi
			fileval=`ls -l $FETCH_PRODUCT_OUT_BACKUP_DIR |wc -l`
			#echo "fileval"
			#echo $fileval
			if [ $fileval -ge 1 ]; then
			#echo "moving to backup"
				`mv $FETCH_PRODUCT_OUT_DIR/* $FETCH_PRODUCT_OUT_BACKUP_DIR`
			fi

echo "End of td_product_output_formatting.sh" >> $LOG_DIR/td_fetch_gl_details_extract.log
echo "#################################################################">> $LOG_DIR/td_fetch_gl_details_extract.log
