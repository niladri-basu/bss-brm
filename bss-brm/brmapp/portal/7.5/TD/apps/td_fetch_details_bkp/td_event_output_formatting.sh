...
FETCH_EVENT_OUT_BACKUP_DIR=`egrep -v "#" login.cfg | grep "FETCH_EVENT_OUT_BACKUP_DIR" |cut -d'=' -f2`
FETCH_EVENT_OUT_DIR=`egrep -v "#" login.cfg | grep "FETCH_EVENT_OUT_DIR" |cut -d'=' -f2`
...
echo "#################################################################" >> $LOG_DIR/td_fetch_gl_details_extract.log
			...
			fileval=`ls -l $FETCH_EVENT_OUT_DIR |wc -l`
			#echo "$fileval"
			if [ $fileval -ge 1 ]; then			
				for file1 in $FETCH_EVENT_OUT_DIR/*; do
					if [ -f "$file1" ]; then 
						filename=${file1##*/}
						...
						#echo $lines
						now=`date +'%Y%m%d%H'`
						target_file=$EXT_OUT_DIR/BRM_NON0_EVENT_EXPORT-$date_run-$now.txt
			...
			fileval=`ls -l $FETCH_EVENT_OUT_BACKUP_DIR |wc -l`
			if [ $fileval -ge 1 ]; then
			#echo "moving to backup"
				`mv $FETCH_EVENT_OUT_DIR/* $FETCH_EVENT_OUT_BACKUP_DIR`
			fi
