...
st.NAME||'*'||dt.NAME||'*'||TO_CHAR(td_unix_to_nz(pd.CYCLE_START_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||pd.STATUS||'*'||TO_CHAR(td_unix_to_nz(pd.CYCLE_END_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||TO_CHAR(td_unix_to_nz(b.LAST_BILL_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||TO_CHAR(td_unix_to_nz(b.NEXT_BILL_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||a.POID_ID0||'*'||b.POID_ID0||'*'||pd.POID_ID0||'*'||s.POID_ID0||'*'||dt.POID_ID0||'*'||pd.NODE_LOCATION
from 
