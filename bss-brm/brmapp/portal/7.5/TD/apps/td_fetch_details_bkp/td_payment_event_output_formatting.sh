...
FETCH_PAY_EVENT_OUT_BACKUP_DIR=`egrep -v "#" login.cfg | grep "FETCH_PAY_EVENT_OUT_BACKUP_DIR" |cut -d'=' -f2`
FETCH_PAY_EVENT_OUT_DIR=`egrep -v "#" login.cfg | grep "FETCH_PAY_EVENT_OUT_DIR" |cut -d'=' -f2`
#ARCHIVE_DIR="$LOG_DIR/ARCHIVE_DIR";

ARCHIVE_DIR=`egrep -v "#" login.cfg | grep "ARCHIVE_DIR" |cut -d'=' -f2`

#Name of the script
SCRIPT_NAME="td_payment_event_output_formatting.sh"
...
echo "#################################################################" >> $LOG_DIR/td_fetch_gl_details_extract.log
echo " Starting formatting output for non zero events `date`" >> $LOG_DIR/td_fetch_gl_details_extract.log
echo "#################################################################" >> $LOG_DIR/td_fetch_gl_details_extract.log

			#cd `$FETCH_PAY_EVENT_OUT_DIR`
			fileval=`ls -l $FETCH_PAY_EVENT_OUT_DIR |wc -l`
			#echo "$fileval"
			if [ $fileval -ge 1 ]; then			
				for file1 in $FETCH_PAY_EVENT_OUT_DIR/*; do
					if [ -f "$file1" ]; then 
						filename=${file1##*/}
