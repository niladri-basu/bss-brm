...
set trimspool on
spool $outfile 
select a.ACCOUNT_NO||'*'||b.BILL_INFO_ID||'*'||b.ACTG_CYCLE_DOM||'*'||st.NAME||'*'||p.NAME||'*'||rb.SCALED_AMOUNT||'*'||TO_CHAR(td_unix_to_nz(pp.CYCLE_START_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||pp.STATUS||'*'||TO_CHAR(td_unix_to_nz(pp.CYCLE_END_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||TO_CHAR(td_unix_to_nz(b.LAST_BILL_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||TO_CHAR(td_unix_to_nz(b.NEXT_BILL_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||rb.GL_ID||'*'||a.POID_ID0||'*'||b.POID_ID0||'*'||pp.POID_ID0||'*'||s.POID_ID0||'*'||p.POID_ID0||'*'||pp.NODE_LOCATION
from 
PIN.ACCOUNT_T a,PIN.SERVICE_ALIAS_LIST_T st,PIN.PURCHASED_PRODUCT_T pp,PIN.SERVICE_T s,PIN.PRODUCT_T p,PIN.RATE_T r,PIN.RATE_PLAN_T rp,PIN.RATE_BAL_IMPACTS_T rb,PIN.BAL_GRP_T bg,PIN.BILLINFO_T b
where
...
--and pp.NODE_LOCATION = 'Plan Charges'
