...
now=`date +'%Y%m%d%H'`
target_dir=`egrep -v "#" login.cfg | grep "FETCH_AMP_OUT_DIR" |cut -d'=' -f2`
log_dir=`egrep -v "#" login.cfg | grep "LOG_DIR" |cut -d'=' -f2`
#$OUT_DIR/BRM_NON0_EVENT_EXPORT-$date_run-$now.txt
filename="amp_extract"
...
st.NAME||'*'||pod.NAME||'*'||TO_CHAR(td_unix_to_nz(pp.CYCLE_START_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||pp.STATUS||'*'||TO_CHAR(td_unix_to_nz(pp.CYCLE_END_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||TO_CHAR(td_unix_to_nz(b.LAST_BILL_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||TO_CHAR(td_unix_to_nz(b.NEXT_BILL_T),'DD-MM-YYYY HH24:MI:SS') ||'*'||a.POID_ID0||'*'||b.POID_ID0||'*'||pp.POID_ID0||'*'||s.POID_ID0||'*'||pt.POID_ID0||'*'||pp.NODE_LOCATION
from
PIN.ACCOUNT_T a,PIN.SERVICE_ALIAS_LIST_T st,PIN.PURCHASED_PRODUCT_T pp,PIN.SERVICE_T s,PIN.PRODUCT_T pt,PIN.BILLINFO_T b,PIN.BAL_GRP_T bg,
