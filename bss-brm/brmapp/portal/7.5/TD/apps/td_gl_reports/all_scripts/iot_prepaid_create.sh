...
outfile=`egrep -v "#" login.cfg | grep "OUTFILE_DIR" |cut -d'=' -f2 |sed -e 's/$/\/iot_prepaid_create.log/'`
...
BEGIN
        ...
        select count(1) into v_count from user_tables where table_name='IOT_GL_PREPAID_BILL_T';

        if v_count <> 0
        then
        v_ddl_stmt := 'DROP TABLE IOT_GL_PREPAID_BILL_T';
        EXECUTE IMMEDIATE v_ddl_stmt;
        end if;

	select count(1) into v_count from user_tables where table_name='TD_GL_PREPAID_REPORT_T';

	if v_count <> 0
        then
        v_ddl_stmt := 'DROP TABLE TD_GL_PREPAID_REPORT_T';
        EXECUTE IMMEDIATE v_ddl_stmt;
        end if;


        v_ddl_stmt :=  'CREATE TABLE IOT_GL_PREPAID_BILL_T (billinfo_poid, bill_poid, start_t, end_t, customer_type, 
			CONSTRAINT pk_iot_prepglbill_x PRIMARY KEY(bill_poid) ) 
			ORGANIZATION INDEX 
			as
	...
	v_ddl_stmt_t :=  'CREATE TABLE TD_GL_PREPAID_REPORT_T
(
