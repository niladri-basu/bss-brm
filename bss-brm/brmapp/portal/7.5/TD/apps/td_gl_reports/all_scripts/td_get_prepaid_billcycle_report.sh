...
# Author : Rakesh Nair
# Date   : 1-Jan-2015
# Descr  : Script to generate prepaid GL report
# Exec   : td_get_prepaid_billcycle_report.sh YYYYMMDs YYYYMMDDe:
#          where YYYYMMDDs should correspond to billing start date
#          and YYYYMMDDe corresponds to billing end date
#          Eg: td_get_prepaid_billcycle_report.sh 20150201 20150301
#=============================================================


LOGINSQL=`egrep -v "#" login.cfg | grep "DB_LOGIN" |cut -d'=' -f2`
DST_VAL=`egrep -v "#" login.cfg | grep "DST_VALUE" |cut -d'=' -f2`
TEMPFILE2=`egrep -v "#" login.cfg | grep "OUTFILE_DIR" |cut -d'=' -f2 |sed -e 's/$/\/td_prepaid_gl_proc.log/'`
TEMPFILE3=`egrep -v "#" login.cfg | grep "OUTFILE_DIR" |cut -d'=' -f2 |sed -e 's/$/\/td_bip_entry.log/'`

if [ "$1" -lt "1" ]
  then echo "Incorrect Execution: Execute as td_get_prepaid_billcycle_report.sh YYYYMMDD"
  exit
fi
...
echo "Starting prepaid GL report generation starting from $bill_start_date: "
date

echo "pin_ledger_report -mode run_report -start $mm/$dd/$yy -type billed -detail -verbose"
pin_ledger_report -mode run_report -start $mm/$dd/$yy -type billed -detail -verbose

echo "Completed prepaid GL report generation at: "
date


echo "Starting td_prepaid_gl_proc.sh  with parameters $start_timestamp and $end_timestamp at:"
date 
sh ./all_scripts/td_prepaid_gl_proc.sh  $start_timestamp $end_timestamp $DST_VAL
...
echo "Completed td_prepaid_gl_proc.sh at: "
date


echo "Starting td_trigger_bip_report.sh at: "
date
sh ./all_scripts/td_trigger_bip_report.sh GL_Report_Prepaid $bill_end_date
