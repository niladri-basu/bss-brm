...
//	int32		flag_value = NULL;
	...
	char		*q = NULL;
	char		*tok = NULL;
	poid_t		*poid1_ptr = NULL;
	poid_t		*poid2_ptr = NULL;
	poid_t		*poid3_ptr = NULL;
	...
	*****************************/
	...
	while (fgets(buf, sizeof(buf), fread) != NULL)
	{
		...
		*/
		...
		str_flist = NULL;
		any_valid_record = 1;
		PIN_POID_DESTROY (poid1_ptr, ebufp);
		PIN_POID_DESTROY (poid2_ptr, ebufp);
		PIN_POID_DESTROY (poid3_ptr, ebufp);
	...
	pin_errbuf_t	*ebufp)
{
	...
	pin_flist_t	*inp_res_flistp = NULL;
	...
	int32 	pp_int = 0;
	int32	off_int = 0;
	pin_decimal_t *amount_dec = NULL;
	pin_decimal_t *quantity_dec = NULL;
	...
	*****************************************/
	...
	//bal_info_flistp = PIN_FLIST_ELEM_GET ( op_in_flistp, PIN_FLD_PRODUCTS, 1, 1, ebufp);
	//inp_res_flistp = PIN_FLIST_ELEM_GET (bal_info_flistp , PIN_FLD_RESULTS, 0,1, ebufp);
	
	//offering_poid = PIN_FLIST_FLD_GET ( bal_info_flistp, PIN_FLD_OFFERING_OBJ, 1, ebufp);	
	//PIN_FLIST_FLD_DROP(op_in_flistp, PIN_FLD_BAL_GRP_OBJ, ebufp);	
	//PIN_FLIST_FLD_DROP(op_in_flistp, PIN_FLD_OFFERING_OBJ, ebufp);	
	...
	/* working in the output*/
	 ...
	 while ((res_flistp = PIN_FLIST_ELEM_GET_NEXT(r_flistp, PIN_FLD_RESULTS, 
		...
		prd_flistp = PIN_FLIST_SUBSTR_GET (res_flistp, PIN_FLD_PRODUCT, 1, ebufp );

		pp_poid = PIN_FLIST_FLD_GET ( prd_flistp, PIN_FLD_OFFERING_OBJ, 1, ebufp) ;


		pp_int = PIN_POID_GET_ID(pp_poid);
		off_int = PIN_POID_GET_ID(offering_poid);	
		account_obj =  PIN_POID_GET_ID((poid_t*)PIN_FLIST_FLD_GET ( res_flistp, PIN_FLD_ACCOUNT_OBJ, 1, ebufp));
		counter++;	
		//if (PIN_POID_COMPARE(pp_poid, offering_poid, 0, ebufp) == 0) {	

		//PIN_ERR_LOG_MSG (PIN_ERR_LEVEL_DEBUG, "Condition Met for offering ");
		counter1++;	
		...
		earned_end_t = *(time_t *)PIN_FLIST_FLD_GET ( res_flistp, PIN_FLD_EARNED_END_T, 1, ebufp);
		 while (( bal_imp_flistp = PIN_FLIST_ELEM_GET_NEXT(res_flistp, PIN_FLD_BAL_IMPACTS,
			...
			if (resource_id == 554 && strcmp(rate_tag, "Tax") != 0) {
				...
				//amount_dec = (pin_decimal_t*)PIN_FLIST_FLD_GET ( bal_imp_flistp, PIN_FLD_AMOUNT, 1, ebufp);
				amount = (char *)pbo_decimal_to_str((pin_decimal_t*)PIN_FLIST_FLD_GET ( bal_imp_flistp, PIN_FLD_AMOUNT,1 , ebufp), ebufp);
				//amount =  (char *)pbo_decimal_to_str(amount_dec, ebufp);
				...
				//quantity_dec  = (pin_decimal_t*)PIN_FLIST_FLD_GET ( bal_imp_flistp, PIN_FLD_QUANTITY,1 , ebufp);	
				//quantity = (char *)pbo_decimal_to_str(quantity_dec, ebufp);
				...
				fprintf((fout), sbuf);
				//pbo_decimal_destroy(&amount_dec);
				//pbo_decimal_destroy(&quantity_dec);
			}
		}
	...
	//fprintf(stdout, "\n\tConnter : %d,%d,%d,%d,%d,%d,%d", account_obj,off_int, pp_int,counter, counter1, counter2, counter3);	
	...
	//PIN_FLIST_DESTROY_EX(&op_in_flistp,NULL);
        ...
        pin_errbuf_t    *ebufp)
{
	...
	*******************************************/
	...
	PIN_FLIST_FLD_SET(*input_flistpp, PIN_FLD_PROGRAM_NAME, (void *)"pin_trial_bill_accts", ebufp);

	//bal_flistp = PIN_FLIST_ELEM_ADD (*input_flistpp, PIN_FLD_PRODUCTS, 1, ebufp);
	...
	//PIN_FLIST_FLD_SET(bal_flistp, PIN_FLD_OFFERING_OBJ, vp, ebufp);
	
	//offering_poid  = (poid_t *)vp; 
