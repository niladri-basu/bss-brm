...
set termout off
spool $outfile 
select /*+ full(et) full(bal) full(it) full(an) parallel(et,4) parallel(bal,4) parallel(it,4) parallel(an,4)*/ 
  ET.ACCOUNT_OBJ_ID0 ||'*'||
...
where
 ET.created_t >= (pin.td_nz_to_unix(TRUNC(SYSDATE-1)))
and ET.created_t < (pin.td_nz_to_unix(TRUNC(SYSDATE)))
...
set auto off
set termout off
select /*+ full(et) full(bal) full(it) full(an) parallel(et,4) parallel(bal,4) parallel(it,4) parallel(an,4)*/ round(sum(bal.amount),2) as SUM_AMOUNT
from 
  PIN.EVENT_T ET,
  PIN.EVENT_BAL_IMPACTS_T BAL,
  PIN.ITEM_T IT,
  pin.account_nameinfo_t an
where
 ET.created_t >= (td_nz_to_unix(TRUNC(SYSDATE-1)))
and ET.created_t < (td_nz_to_unix(TRUNC(SYSDATE)))
and ET.poid_type in $event_poid_type
and ET.POID_ID0 = BAL.OBJ_ID0
and BAL.AMOUNT != 0
and IT.POID_ID0 = BAL.ITEM_OBJ_ID0
