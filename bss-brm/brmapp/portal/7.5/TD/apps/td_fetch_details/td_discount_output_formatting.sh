			...
			if [ $fileval -ge 1 ]; then			
				for file1 in $FETCH_DISCOUNT_OUT_DIR/*; do
					if [ -f "$file1" ]; then 
						...
						echo $lines
						#now=`date +'%Y%m%d%H'`
						int_file=$EXT_OUT_DIR/BRM_PLAN_DISCOUNT_EXPORT_int.txt
						int1_file=$EXT_OUT_DIR/BRM_PLAN_DISCOUNT_EXPORT_int1.txt
						target_file=$EXT_OUT_DIR/BRM_PLAN_DISCOUNT_EXPORT-$date_run.txt
						#`cat $file1 | sed 's/ //g' >> $target_file `
						#`cp $file1 $target_file`
						#`sed -i 'Total number of rows = ' $target_file`
						`cat $file1 | sed 's/ \+ /\t/g'| sed 's/\t//g' |sed 's/\* /*/g' >> $target_file `
						#`cat $int_file | sed 's/\t//g' >> $int1_file `
						#`cat $int1_file | sed 's/\* /*/g' >> $target_file `
						
						echo "Record_count*$count" >>$target_file
						sed -i '1 s/^/ACCOUNT_NO*BILL_INFO_ID*BILL_CYCLE*MSISDN*DISCOUNT_NAME*DISCOUNT_PERCENT*DISCOUNT_START_DATE*PP_STATUS*DISCOUNT_END*LAST_BILL_DATE*NEXT_BILL_DATE*ACCOUNT_POID*BILL_INFO_POID*PURCHASE_DISC_POID*SERVICE_POID*DISCOUNT_POID*DISC_NODE_LOCATION\n/' $target_file
