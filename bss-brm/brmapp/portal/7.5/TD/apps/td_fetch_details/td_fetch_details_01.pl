...
$FETCH_REALTIME_EVENT_OUT_DIR = "$PIN_HOME/TD/apps/td_fetch_details/realtime_event_out";
...
$FETCH_REALTIME_EVENT_OUT_BACKUP_DIR = "$PIN_HOME/TD/apps/td_fetch_details/realtime_event_out_backup";
...
if  ( ! -e $FETCH_REALTIME_EVENT_OUT_DIR)
{
        mkdir ("$FETCH_REALTIME_EVENT_OUT_DIR", 0777) || die "Unable to create $FETCH_REALTIME_EVENT_OUT_DIR directory";
        print "Creating log directory $FETCH_REALTIME_EVENT_OUT_DIR \n";
}
if  ( ! -e $FETCH_REALTIME_EVENT_OUT_BACKUP_DIR)
{
        mkdir ("$FETCH_REALTIME_EVENT_OUT_BACKUP_DIR", 0777) || die "Unable to create $FETCH_REALTIME_EVENT_OUT_BACKUP_DIR directory";
        print "Creating log directory $FETCH_REALTIME_EVENT_OUT_BACKUP_DIR \n";
}
if  ( ! -e $ARCHIVE_DIR)
{
        mkdir ("$ARCHIVE_DIR", 0777) || die "Unable to create $ARCHIVE_DIR directory";
        print "Creating log directory $ARCHIVE_DIR \n";
}
...
print MAINLOG "Starting to collect realtime event data from database on $datetime!!!\n";
system("sh ./fetch_realtime_event_details.sh");
if ($? == 256) {
    print "Exit\n";
    exit;
}
			
print MAINLOG "Fetching of realtime event data completed on $datetime!\n";
print MAINLOG "Starting archiving and renaming realtime event data file!\n";
system("sh ./td_realtime_event_output_formatting.sh");
if ($? == 256) {
    print "Exit\n";
    exit;
}

print MAINLOG "Archiving and renaming realtime event data file and sending to out directory completed! on $datetime!\n";
...
print MAINLOG "Archiving and renaming Product data file and sending to out directory completed! on $datetime!\n";
