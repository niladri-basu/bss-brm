...
#
# Create the FS_BILL_CONTROL_LOG_DIR and FS_BILL_CONTROL_LOG_DIR if it doesn't exist.
#

if  ( ! -e $FS_BILL_CONTROL_LOG_DIR )
{
        mkdir ("$FS_BILL_CONTROL_LOG_DIR", 0777) || die "Unable to create $FS_BILL_CONTROL_LOG_DIR directory";
        print "Creating log directory $FS_BILL_CONTROL_LOG_DIR \n";
}
if  ( ! -e $FS_BILL_CONTROL_OUT_DIR )
{
        mkdir ("$FS_BILL_CONTROL_OUT_DIR", 0777) || die "Unable to create $FS_BILL_CONTROL_OUT_DIR directory";
        print "Creating log directory $FS_BILL_CONTROL_OUT_DIR \n";
}
...
open (MAINLOG, ">>$FS_BILL_CONTROL_LOG_DIR/fs_control_logs.log") || die "Unable to create file $FS_BILL_CONTROL_LOG_DIR/fs_control_logs.log";
