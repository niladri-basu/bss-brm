...
#
# Script: rollback_suspense_preprocess.pl
#
# Description:
#   This script is used to generate bulk loader files to update suspended usage records in case 
#   a recycle operation is rolled back in the pipeline.
#
# Revision: 2005-02-16
#
...
$suspRecType = "720";		# suspense record type
...
if($SQL_SERVER_SUPPORT == 1)
{
  $STATUS              =               0;
  $SUSPENSE_REASON     =               0;
  $SUSPENSE_SUBREASON  =               0;
  $ERROR_CODE          =               0;
  $RECYCLE_MODE        =               0;
  $POID_REV            =               0;
  $READ_ACCESS         =               "L";
  $WRITE_ACCESS        =               "L";
  $POID_TYPE           =               "/tmp_suspended_usage";

	$lineConst =  $deli.$STATUS.$deli.$SUSPENSE_REASON.$deli.$SUSPENSE_SUBREASON.$deli.$ERROR_CODE.$deli.
			$RECYCLE_MODE.$deli.$POID_REV.$deli.$READ_ACCESS.$deli.$WRITE_ACCESS.$deli.$createdTime.
			$deli.$modifiedTime.$deli.$poidDb.$deli.$POID_TYPE;
