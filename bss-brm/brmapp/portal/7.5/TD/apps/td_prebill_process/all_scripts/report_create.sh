...
BEGIN
        ...
        if v_count <> 0
        then
	v_dml_stmt := 'insert /*+ APPEND */ into TD_PREBILL_REPORT_HIST_T 
select /*+ full(a) */  sysdate,
a.ACTG_DOM, a.LAST_BILL_T, a.NEXT_BILL_T, a.ACCOUNT_NO, a.CUST_SEG_DESC, 
a.BILL_INFO_ID, a.NAME, a.TAX_CODE, a.CHARGE_FROM, a.CHARGE_TO, a.OVERRIDE_FLAG, 
a.OVERRIDE_AMT, a.FIX_AMOUNT, a.SCALED_AMOUNT, a.PRORATE_FIRST, 
a.CYCLE_FEE_FLAGS, a.MSISDN, a.P_FLAGS, a.ANTICIPATED_AMOUNT
from TD_PREBILL_REPORT_T a';
 ...
 v_ddl_stmt :=  'create table td_prebill_report_t as
                 select td.ACTG_DOM, td.LAST_BILL_T, td.NEXT_BILL_T, td.ACCOUNT_NO, td.CUST_SEG_DESC, td.BILL_INFO_ID, td.NAME, td.TAX_CODE, td.CHARGE_FROM, td.CHARGE_TO, td.OVERRIDE_FLAG, td.OVERRIDE_AMT, td.FIX_AMOUNT, td.SCALED_AMOUNT, td.PRORATE_FIRST, td.CYCLE_FEE_FLAGS, td.MSISDN, td.P_FLAGS,td.node_location,
                 case
