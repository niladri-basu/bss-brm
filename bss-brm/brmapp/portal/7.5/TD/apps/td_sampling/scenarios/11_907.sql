select distinct iot.billinfo_poid_id0 ||',11_907'
...
and pdt.status=1
and dt.name='$15 Discount for $70 and up'
and floor(months_between(TO_DATE(trunc(unix_to_nzdate(bt.next_BILL_T)),'dd/mm/yyyy'),
                        TO_DATE(trunc(unix_to_nzdate(pdt.cycle_start_t)),'dd/mm/yyyy'))) < 1;
