...
# 
#@(#)$Id: modify_schema_75ps7.pl /cgbubrm_7.5.0.rwsmod/1 2013/11/14 01:54:26 mkothari Exp $
#
# modify_columns_75ps7.pl
# 
# Copyright (c) 2010, 2013, Oracle and/or its affiliates. All rights reserved.
#
#    NAME
#      modify_columns_75ps7.pl - This module will result in creating missing indexes . 
#
#    DESCRIPTION

#	These scripts will be called one by one by this Perl File
#	modify_columns_75ps7.pl for execution.
#
#    NOTES
#	The Driver File for upgrade which is the script "pin_upgrade_pipeline_75ps7.pl"
#	will call this perl script (modify_columns_75ps7.pl)  through one of its wrappers APIs
#	named modify_columns_75ps7() .

#    ERROR HANDLING
#	The Error Handling will be done by logging the error messages in the log file.
#	This log file is modify_columns_75ps7.log which will be generated in $IFW_HOME/log directory.

#    MODIFIED   (MM/DD/YY)
#    mkothari    11/14/13 - Creation
#
use warnings;
use Env;
...
our @ALL_MODIFY_SCRIPTS_75PS7 = ("modify_ifw_networkoper_PS7.source");

sub modify_columns_75ps7 {
        my $file = "";
        my $time_stamp = "";
        my $addnewindfile = "$IFW_LOG_DIR/modify_columns_75ps7.log";
        my $ReadString = "";
        my $Result    = 0;
        my $FirstElem = $ALL_MODIFY_SCRIPTS_75PS7[0];
        my $writestatement = "";
        ...
        if ( $FirstElem eq "" ) {
                ...
                $SETUP_LOG_FILENAME = "$IFW_LOG_DIR/tmp_modify_columns_75ps7.log";
                $SETUP_LOG_ACCESS_TYPE = "WRITE";
                &OpenLogFile( $SETUP_LOG_FILENAME, $SETUP_LOG_ACCESS_TYPE );
        
	        foreach $file (@ALL_MODIFY_SCRIPTS_75PS7) {
