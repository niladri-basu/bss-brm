...
#
# @(#)$Id: pin_upgrade_pipeline_75ps3.pl /cgbubrm_7.5.0.rwsmod/1 2012/09/17 06:10:13 hsnagpal Exp $
#
# $Header: bus_platform_vob/upgrade/Integrate/75_75ps3/oracle/pin_upgrade_pipeline_75ps3.pl /cgbubrm_7.5.0.rwsmod/1 2012/09/17 06:10:13 hsnagpal Exp $
#
# pin_75ps3_pipeline_upgrade.pl
#
# Copyright (c) 2009, 2012, Oracle and/or its affiliates. All rights reserved. 
#
#    NAME
#      pin_75ps3_pipeline_upgrade.pl - this script is the main driver file
...
print "now executing pipeline upgrade module ... \n";
	...
	if ( $ret_val == 0 ) {
		print "pipeline upgrade to BRM 75ps3 is successful. \n";
		print
	"please see the pipeline upgrade log file at $IFW_LOG_DIR/pin_75ps3_pipeline_upgrade.log \n";
		print "################################################ \n";
	} elsif ( $ret_val == 2 ) {
		print
...
sub pipeline_db_upgrade_75 {
	...
	my $api_result  = -1;
	my $Result      = -1;
	my $pipelineupgrade75pinlog = "$IFW_LOG_DIR/pin_75ps3_pipeline_upgrade.log";
	my $writestatement = "";
	...
	if ( $firstelem eq "" ) {
		...
		foreach $upgrade_api (@EXECUTE_ALL_PIPELINE_UPGRADE_SCRIPTS) {
			...
			print "pipeline upgrade from 75ps3 is now executing api:$upgrade_api \n";
