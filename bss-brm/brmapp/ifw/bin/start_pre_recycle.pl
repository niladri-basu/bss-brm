...
#	
#########################################################################################
# Utility script for PipeLine ---> Start PRE_RECYCLE Pipeline
#########################################################################################
...
sub start_pre_recycle {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/start_pre_recycle.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for start_pre_recycle is sent \n";
}
