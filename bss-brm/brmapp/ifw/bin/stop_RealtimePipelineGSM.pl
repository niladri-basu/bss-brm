...
#	
#########################################################################################
# Utility script for PipeLine ---> Stop RealtimePipelineGSM Pipeline
#########################################################################################
...
sub stop_RealtimePipelineGSM {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/stop_RealtimePipelineGSM.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for stop_RealtimePipelineGSM is sent \n";
}

stop_RealtimePipelineGSM;
