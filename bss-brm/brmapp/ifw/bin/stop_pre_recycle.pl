...
#	
#########################################################################################
# Utility script for PipeLine ---> Stop PRE_RECYCLE Pipeline
#########################################################################################
...
sub stop_pre_recycle {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/stop_pre_recycle.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for stop_pre_recycle is sent \n";
}
