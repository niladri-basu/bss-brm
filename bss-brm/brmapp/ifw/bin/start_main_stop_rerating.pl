...
#	
#########################################################################################
# Utility script for PipeLine ---> Start Main Stop Rerating
#########################################################################################
...
sub start_main_stop_rerating {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/start_main_stop_rerating.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for start_main_stop_rerating is sent \n";
}

start_main_stop_rerating;
