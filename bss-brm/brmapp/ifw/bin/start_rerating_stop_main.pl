...
#	
#########################################################################################
# Utility script for PipeLine ---> Start Rerating Stop Main
#########################################################################################
...
sub start_rerating_stop_main {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/start_rerating_stop_main.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for start_rerating_stop_main is sent \n";
}

start_rerating_stop_main;
