...
#	
#########################################################################################
# Utility script for PipeLine ---> Start DiscountPipeline Pipeline
#########################################################################################
...
sub start_DiscountPipeline {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/start_DiscountPipeline.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for start_DiscountPipeline is sent \n";
}

start_DiscountPipeline;
