...
#	
#########################################################################################
# Utility script for PipeLine ---> Stop DiscountPipeline Pipeline
#########################################################################################
...
sub stop_DiscountPipeline {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/stop_DiscountPipeline.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for stop_DiscountPipeline is sent \n";
}

stop_DiscountPipeline;
