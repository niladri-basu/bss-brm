...
#	
#########################################################################################
# Utility script for PipeLine ---> Start All Pipeline
#########################################################################################
...
sub start_all_pipeline {
  if("a$ENV{'IFW_HOME_PATH'}" eq "a")
  {
    print "Pipeline home not found, aborting....\n";
    return 1;
  }
  open(INSTRM, "$ENV{'IFW_HOME_PATH'}/semaphore/start_all_pipeline.reg") || die "Cannot open file :$!";
  ...
  print "Semaphore for start_all_pipeline is sent \n";
}
