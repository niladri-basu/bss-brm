 ...
 //logStdout( edrString( DETAIL.B_NUMBER ) + "\n" );
	  ...
	  //--------------------------------------------------------------------
		...
		outputWrite(">#");
              ...
              //-------------------------------------------------
              // New Fields Added as part of Data Clock changes 
              //-------------------------------------------------

              edrAddDatablock(DETAIL.ASS_INVOICE_DATA.PACKAGE_GROUP_TEST);
              if (  edrString( DETAIL.BASIC_SERVICE ) == "VAP")
              {
                for ( j = 0; j < edrNumDatablocks( DETAIL.ASS_CI.ASS_VV, 0 ); j = j + 1 )
                {
                   edrString(DETAIL.PK_GROUP_DISPLAY) = edrString(DETAIL.ASS_CI.ASS_VV.PK_GROUP_DISPLAY, 0, j);
                }
              }

              else if (  edrString( DETAIL.BASIC_SERVICE ) == "VAS")
              {
                for ( j = 0; j < edrNumDatablocks( DETAIL.ASS_CI.ASS_VAS, 0 ); j = j + 1 )
                {
                   edrString(DETAIL.PK_GROUP_DISPLAY) = edrString(DETAIL.ASS_CI.ASS_VAS.PK_GROUP_DISPLAY, 0, j);
                }
              }
		else
		{
			edrString(DETAIL.PK_GROUP_DISPLAY) = "";
		}

              //------------------------------------------------------------------------------------------
              // New Fields Originating and Destination Country introduced as part of Roaming CSV changes 
              //------------------------------------------------------------------------------------------
                edrString(DETAIL.ORIGINATING_COUNTRY) = edrString(DETAIL.ASS_CI.ORIGINATING_COUNTRY, 0);
                edrString(DETAIL.DESTINATION_COUNTRY) = edrString(DETAIL.ASS_CI.DESTINATION_COUNTRY, 0);

              	edrOutputMap("V670_EVENT_LOADER.PACKAGE_GROUP_TEST.STD_MAPPING");
